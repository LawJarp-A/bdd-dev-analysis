...
def compute_coco_metrics() -> dict:
    if not COCO_ANN.exists():
        raise FileNotFoundError(f"COCO annotations not found: {COCO_ANN}")
    if not PRED_JSON.exists():
        raise FileNotFoundError(
            f"Predictions not found: {PRED_JSON}  (run inference first)"
        )
...
def _compute_all_stats(
    ...
    """Compute confusion matrix, per-image stats, and per-class failure clusters."""
    ...
    for img_name, img_gts in val_df.groupby("image_name"):
        ...
        for cat, cat_gts in img_gts.groupby("category"):
            ...
            cat_recall = (
                len(cat_matched) / len(cat_gt_boxes) if len(cat_gt_boxes) > 0 else 0.0
            )
            class_rows.append(
                {
                    "image_name": img_name,
    ...
    if METRICS_PATH.exists():
        per_image = per_image.merge(
            pd.read_csv(METRICS_PATH), on="image_name", how="left"
        )
...
def build_cache() -> dict:
    ...
    print("Computing confusion matrix and failure analysis...")
    ...
    failure_df = per_image.merge(
        img_stats, on="image_name", how="left", suffixes=("", "_gt")
    )
    ...
    # Correlation between image features and recall
