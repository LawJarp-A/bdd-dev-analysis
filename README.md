...
**Goal:** Understand the BDD100K dataset before training. Identify class imbalances, annotation quality issues, and edge cases that could affect model performance.
...
**RF-DETR on BDD100K validation set:**
...
**The model performs well on frequent, large classes and struggles with rare, small ones.** Car leads at 81.4% AP@50 while train sits at 6.3% with only 15 validation samples. The gap between small object AP (13.4%) and large object AP (61.6%) is 4.6x, confirming that BDD100K's extreme scale variation is a major challenge.

**Phase 1 findings directly predicted Phase 3 results.** Class imbalance predicted which classes would underperform. High occlusion rates for riders (89%) and bikes (84%) mapped to their lower AP scores (51.3% and 54.2%). The dataset's bias toward clear/daytime conditions showed up as higher failure rates in rainy+night scenarios (37.1% failure rate vs much lower in clear/daytime).

**Failure clusters reveal actionable patterns.** Rainy + night + tiny objects is the hardest scenario. Riders at night fail more than cars at night because riders are smaller and rarer. Crowded scenes hurt recall (r=-0.26), and darker images are harder (brightness vs recall r=0.16).
...
6. **Per-image precision/recall/F1.** Computed per image rather than per class, then grouped by weather, time of day, and object size to find failure clusters. This is where Phase 1 connects to Phase 3: rainy + night + tiny objects has the highest failure rate (37.1%), while clear + daytime (the dominant condition in training) has much lower failure rates. This confirms the Phase 1 finding that the dataset is biased toward clear/daytime conditions.

7. **Correlation analysis.** Pearson and Spearman correlations between image-level features and recall: object area vs recall (r=0.27, larger objects are easier to detect), GT count vs recall (r=-0.26, crowded scenes hurt recall), brightness vs recall (r=0.16, darker images are harder), occlusion rate vs recall (r=-0.08, occluded objects slightly harder).
...
The dashboard explicitly links data analysis findings to model performance. Class imbalance (Phase 1) directly predicts AP@50 (Phase 3), where rare classes perform worse. High occlusion rates (Phase 1) correlate with lower AP for those classes (Phase 3). Weather/time bias in training data (Phase 1) maps to higher failure rates in underrepresented conditions (Phase 3).
...
│   └── convert_to_coco.py     # BDD100K to COCO format conversion
└── evaluation/
